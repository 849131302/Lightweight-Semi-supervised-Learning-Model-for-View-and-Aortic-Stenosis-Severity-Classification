...
def fastvit_t8(pretrained=False, **kwargs):
    ...
    if pretrained:
        #url="https://docs-assets.developer.apple.com/ml-research/models/fastvit/image_classification_distilled_models/fastvit_t8_reparam.pth.tar"
        url ='https://docs-assets.developer.apple.com/ml-research/models/fastvit/image_classification_models/fastvit_t8.pth.tar'
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location=None, check_hash=True)#True
...
def fastvit_sa24(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location=None, check_hash=True)
        print(checkpoint.keys())
        model.load_state_dict(checkpoint["state_dict"])
    return model
...
class MARCLinear(nn.Module):
    ...
    def forward(self, input, *args):
        ...
        return logit_after
class MARCFastViT(nn.Module):
    def __init__(self, pretrained=False, **kwargs):
        super(MARCFastViT, self).__init__()
        ...
        downsamples = [True, True, True, True]
        token_mixers = ("repmixer", "repmixer", "repmixer", "repmixer")
        # Create a FastViT model with MARCLinear layer
        self.model = FastViT(
            layers,
            token_mixers=token_mixers,
            embed_dims=embed_dims,
        ...
        self.model.default_cfg = default_cfgs["fastvit_t"]
        if pretrained:
            url = 'https://docs-assets.developer.apple.com/ml-research/models/fastvit/image_classification_models/fastvit_t8.pth.tar'
            checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location=None, check_hash=False)
            self.model.load_state_dict(checkpoint["state_dict"])
        in_features = 768
        out_features = 3
        self.model.head = MARCLinear(in_features, out_features)
    def forward(self, x):
        return self.model(x)