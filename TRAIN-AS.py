...
from models.lwn import reparameterize_model, LWN
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if use_amp:
            with torch.cuda.amp.autocast(): #自动混合精度的计算
                loss = torch.nan_to_num(criterion_train(output, targets))  # 计算训练损失，并使用 torch.nan_to_num 将 NaN 替换为 0（防止 NaN 对训练造成问题）。
...
if __name__ == '__main__':
    #创建保存模型的文件夹
    file_dir =os.path.abspath('checkpoints/TMED-2-AS/7E-4mixup_alpha=0.8,PROB=1.0,')
    ...
    # 设置全局参数
    model_lr = 7e-4#3 4VIEWTRAIN 5E-4
    BATCH_SIZE = 16
    EPOCHS = 200
    ...
    mixup_fn = Mixup(
        mixup_alpha=0.8, cutmix_alpha=1.0, cutmix_minmax=None,  # mixup_alpha=0, cutmix_alpha=1.0更好
        prob=1.0, switch_prob=0.5, mode='batch',
        label_smoothing=0.1, num_classes=classes)  # 标签平滑时，适当的值通常在 [0, 0.1] 范围内
    # 读取数据
    # dataset_train = datasets.ImageFolder('/home/CV/AS/TMED1/TMED-156-52_fold2/train', transform=transform)#target_transform=map_labels(class_to_idx)
    # dataset_val = datasets.ImageFolder("/home/CV/AS/TMED1/TMED-156-52_fold2/val", transform= transform_val)#,target_transform=map_labels(class_to_idx)
    dataset_train = datasets.ImageFolder('D:\CV\LWM-SSL\TMED\TMED-2\TMED-AS\DEV479\\train', transform=transform)  # target_transform=map_labels(class_to_idx)
    dataset_val = datasets.ImageFolder('D:\CV\LWM-SSL\TMED\TMED-2\TMED-AS\DEV479\\val',transform=transform_val)  # ,target_transform=map_labels(class_to_idx)
    ...
    # 实例化模型并且移动到GPU
    criterion_train = SoftTargetCrossEntropy()#MultiClassFocalLossWithAlpha()#MultiCEFocalLoss(class_num=3, gamma=2, alpha=0.25)#  # SoftTargetCrossEntropy()
