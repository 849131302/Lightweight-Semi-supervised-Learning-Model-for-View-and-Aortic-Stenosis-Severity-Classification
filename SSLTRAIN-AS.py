...
from models.lwn import LWN, reparameterize_model
...
def get_config():
    ...
    parser.add_argument("--save_name", type=str, default='FreeMatch-as-200EPOCH3E-3spli3-2RATIO224-0.8-1,0')#softmatch_fastvit
    parser.add_argument("--save_dir",  type=str, default='./FreeMatch-TMED-1')
    parser.add_argument("--resume", type=str,default=None)#''/home/CV/AS/checkpoints/不归一 无预训练，TMED自己训练/model_35_78.306.pth'
    parser.add_argument("--EMA_decay", type=float,default=0.9995)  # 是用于控制概率分布的 EMA 的 momentum 参数,该参数通常用于计算无标签数据的伪标签（pseudo labels）的概率分布。0.999
...
if __name__ == '__main__':
    ...
    args = get_config()
    class_weights = (torch.tensor([0.35, 0.4, 0.25])).cuda(0)



    labeled_data_path = 'D:\CV\LWM-SSL\TMED\TMED-1\TMED-156-52_fold2\\train'
    unlabeled_data_path = 'D:\CV\LWM-SSL\TMED\TMED-1\\Unlabeled Set'
    val_data_path = 'D:\CV\LWM-SSL\TMED\TMED-1\TMED-156-52_fold2\\val'
    test_data_path = 'D:\CV\LWM-SSL\TMED\TMED-1\TMED-156-52_fold2\\test'
    ...
    strong_transform = transforms.Compose([
        ...
        TargetedStrongAugment(n=4, window_size=112),
        transforms.ToTensor(),
        # transforms.Normalize(0.061, 0.140)
    ])
